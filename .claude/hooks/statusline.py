...
from common_hook import normalize_task_ref, resolve_task_dir  # type: ignore[import-not-found]
...
def _get_current_task(trellis_dir: Path) -> dict | None:
    ...
    task_ref = normalize_task_ref(_read_text(trellis_dir / ".current-task"))
    if not task_ref:
        return None

    # Resolve task directory
    task_path = resolve_task_dir(trellis_dir, task_ref)
